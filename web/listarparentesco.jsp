...
<%@page import="Modelos.parentesco"%>
...
<html>
    ...
    <%
        List<parentesco> parentesco = (List<parentesco>)request.getAttribute("parentesco");
    %>
    <body>
        ...
        <table  class="table table-striped table-bordered table-sm container" >
            ...
            <%
                for(parentesco parentescotemporal : parentesco)
                {
                    %>
                    <tr>
                        <td><%=parentescotemporal.getIdparentesco()%></td>
                        <td><%=parentescotemporal.getId_alumno()%></td>
                        <td><%=parentescotemporal.getId_apoderado2()%></td>
                        <td><%=parentescotemporal.getTipo()%></td>
    
                       
                        <td>
                            <a href="salumno?accion=Cargar&idparentesco=<%=parentescotemporal.getIdparentesco()%>">Actualizar</a>
                            &nbsp;|&nbsp;
                            <a href="salumno?accion=Eliminar&idparentesco=<%=parentescotemporal.getIdparentesco()%>">Eliminar</a>
                        </td>
            ...
            <div class="container">
                 <input type="button" value="Nuevo Registro" class="btn btn-success" onclick="location.href='registrarparentesco.jsp'"/>
            </div>
            <div class="container">
