...
<html>
    ...
    <%
        List<alumno> alumno = (List<alumno>)request.getAttribute("alumno");
    %>
    <body>
        ...
        <table  class="table table-striped table-bordered table-sm container" >
            ...
            <%
                for(alumno alumnotemporal : alumno)
                {
                    %>
                    <tr>
                        <td><%=alumnotemporal.getDnialumno()%></td>
                        <td><%=alumnotemporal.getNombre() %></td>
                        <td><%=alumnotemporal.getApellidopaterno()%></td>
                        <td><%=alumnotemporal.getApellidomaterno()%></td>
                        <td><%=alumnotemporal.getFechanac()%></td>
                        <td><%=alumnotemporal.getEdad()%></td>
                        <td><%=alumnotemporal.getDireccion()%></td>
                        <td><%=alumnotemporal.getCel()%></td>
                        <td><%=alumnotemporal.getCorreo()%></td>
                        <td>
                            <%
                            if(alumnotemporal.getSexo().equals("M"))
                            {
                                %>Masculino<%
                            }
                            else
                        ...
                        <td><%=alumnotemporal.getObservaciones()%></td>
                        <td><%=alumnotemporal.getReligion()%></td>
                        <td><%=alumnotemporal.getFechareg()%></td>
                        <td>
                            <a href="salumno?accion=Cargar&idalumno=<%=alumnotemporal.getIdalumno()%>">Actualizar</a>
                            &nbsp;|&nbsp;
                            <a href="salumno?accion=Eliminar&idalumno=<%=alumnotemporal.getIdalumno()%>">Eliminar</a>
                        </td>
            ...
            <div class="container">
                 <input type="button" value="Nuevo Registro" class="btn btn-success" onclick="location.href='registraralumno.jsp'"/>
            </div>
            <div class="container">
